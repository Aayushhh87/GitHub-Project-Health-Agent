...
def test_quality_detects_python_syntax_error() -> None:
    ...
    assert any(
        finding.title == "Python syntax error"
...
def test_quality_detects_javascript_debug_statement() -> None:
    ...
    assert any(
        finding.title == "Debug statement in source"
        for finding in findings
    )
...
def test_quality_detects_python_print_statement() -> None:
    findings = analyze_quality(
        snapshot_for(
            "src/debug.py",
            "def hello():\n"
